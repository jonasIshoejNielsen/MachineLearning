...
def compute_histogram(magnitude, orientation, bins=8, isPlotting=False):
    ...
    bins_number = bins
    thetas = np.linspace(0.0, 360.0, bins_number + 1)

    # Compute the histogram based on the orientation and magnitude.
    hist, _, _ = hist_ax.hist(orientation.ravel(), thetas, weights=magnitude.ravel())
    hist_ax.cla()

    # Plot the polar histogram.
    if isPlotting:
        bars = hist_ax.bar(thetas[:bins_number], hist, bottom=0.0)
        for bar in bars:
            bar.set_alpha(0.5)
        plt.draw()
...
def compute_gradients(image):
    ...
    Gx = cv2.Sobel(grayscale, cv2.CV_64F, 1, 0, ksize=1)
    Gy = cv2.Sobel(grayscale, cv2.CV_64F, 0, 1, ksize=1)
...
def compute_hog():
    ...
    # Compute HOG.
    (H, hog) = feature.hog(hog, orientations=bins, pixels_per_cell=(pixels_per_cell, pixels_per_cell),
                           cells_per_block=(cells_per_block, cells_per_block), block_norm="L2",
